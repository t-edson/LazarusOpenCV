...
type
  ...
  *)
  TCV_SEQUENCE_FIELDS = record
    CV_TREE_NODE_FIELDS: array of TCvSeq;//TCV_TREE_NODE_FIELDS<TCvSeq>;
    total: Integer; (* Total number of elements. *)
    elem_size: Integer; (* Size of sequence element in bytes. *)
...
begin
  // (((elemtype*)((image)->imageData + (image)->widthStep*(row)))[(col)])
  Result := {$IFDEF DELPHI7}Pointer({$ENDIF DELPHI7}{$IFDEF DELPHI2005_UP}PByte{$ELSE}{$IFDEF FPC}Pointer({$ELSE}Integer{$ENDIF}{$ENDIF}(image^.imageData) + image^.widthStep *
